...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function displayKnowledgeBaseEntries(serverName, jobName) {
        ...
        // Rensa eventuell gammal container
    ...
    const acknowledgeJob = async (jobName) => {
        ...
        try {
            await fetch('/api/acknowledge', {
             ...
             // Låt socket-uppdateringen hantera UI
        } catch (error) {
    ...
    // ============================================================================
    // === NY SEKTION FÖR INDEXANALYS STARTAR HÄR ===============================
    // ============================================================================

    /**
     * Anropar backend för att generera ett index-skript och kopierar det till urklipp.
     */
    ...
    /**
     * Hämtar och renderar data för både saknade och oanvända index.
     */
    ...
    // ============================================================================
    // === NY SEKTION FÖR INDEXANALYS SLUTAR HÄR ================================
    // ============================================================================
    ...
    // Event listener för "Skapa Skript"-knapparna (använder delegering)
  ...
  // ============================================================================
    ...
    const initialize = async () => {
        try {
            ...
            if (servers.length > 0) {
                ...
                // Socket.IO kommer automatiskt att skicka 'initialStatus' vid anslutning,
                // vilket triggar startDataFetchingForCurrentServer.
            } else {
    ...
    socket.on('initialStatus', (initialData) => {
        ...
        for (const serverName in initialData) {
            if (initialData[serverName].hasBlocking) {
                showAlarmBanner(serverName);
                break; // Visa bara en banner åt gången
            }
        }
        // Om en server redan är vald, starta datahämtningen
        if (currentServer) {
            startDataFetchingForCurrentServer();
        }
    });
    ...
    kbForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            alert('Lösningen har sparats!');dbName: index.TableName.split('.')[0].replace(/[<span class="katex-display"><span class="katex"><span class="katex-mathml"><math xmlns="http://www.w3.org/1998/Math/MathML" display="block"><semantics><mrow></mrow><annotation encoding="application/x-tex"></annotation></semantics></math></span><span class="katex-html" aria-hidden="true"></span></span></span>]/g, ''),
            closeKbModal();
